...
\lstdefinelanguage{javascript}{
  ...
  keywords=[2]{true, false, null, undefined},
  sensitive=true,
  comment=[l]{//},
  morecomment=[s]{/*}{*/},
  morestring=[b]",
...
\author{\textbf{Priyanshu Kumar Sharma, Vaishnavi Jadhav, Vaibhav Gulge}}
\date{\today}
...
\begin{titlepage}
    \begin{center}
        ...
        \Large
        \textbf{Priyanshu Kumar Sharma, Vaishnavi Jadhav, Vaibhav Gulge}
...
\noindent This is to certify that the project report entitled \textbf{``Next-Generation Immersive Digital Learning Platform''} submitted by \textbf{Priyanshu Kumar Sharma, Vaishnavi Jadhav, Vaibhav Gulge} in partial fulfillment of the requirements for the award of the degree of \textbf{Bachelor of Technology} in \textbf{Computer Science and Engineering} is a bona fide record of the work carried out under my supervision and guidance.
...
\noindent This is to certify that the thesis entitled \textbf{``Next-Generation Immersive Digital Learning Platform''} submitted by \textbf{Priyanshu Kumar Sharma, Vaishnavi Jadhav, Vaibhav Gulge} to University Name for the award of the degree of \textbf{Bachelor of Technology} is a record of bona fide research work carried out by him under my supervision and guidance. The results embodied in this thesis have not been submitted to any other University or Institute for the award of any degree or diploma.
...
\noindent We, \textbf{Priyanshu Kumar Sharma, Vaishnavi Jadhav, and Vaibhav Gulge}, hereby declare that the project report entitled \textbf{``Next-Generation Immersive Digital Learning Platform''} submitted to University Name in partial fulfillment of the requirements for the award of the degree of \textbf{Bachelor of Technology} in \textbf{Computer Science and Engineering} is a record of original work done by us under the supervision and guidance of \textbf{Prini Rastogi}. This project work has not been submitted elsewhere for any other degree or diploma.

\vspace{3cm}

\noindent \textbf{Priyanshu Kumar Sharma} \\
\textbf{Vaishnavi Jadhav} \\
\textbf{Vaibhav Gulge} \\
Roll Number
Date: \today
...
We are also grateful to \textbf{HOD Name}, Head of the Department of Computer Science and Engineering, for providing the necessary facilities and support.
...
\textbf{Teacher Functionality} empowers educators to deliver engaging content and assess student progress effectively. The centerpiece is the \textbf{Virtual Classroom Management} suite. Teachers can schedule live sessions, which automatically generates unique, secure meeting IDs. During these sessions, they have granular control over the environment, including the ability to mute participants, share their screen, and track attendance automatically based on join/leave times. The \textbf{Assessment System} allows for the creation of rich, interactive quizzes. Unlike standard forms, these quizzes support various question types including multiple-choice and code-based problems. The system automatically grades objective questions and uses AI to suggest feedback for subjective answers, significantly reducing the grading burden.

\textbf{Student Functionality} is designed to support autonomous, engaging learning. The \textbf{AI Tutoring System} is always available to answer questions. Students can upload documents or simply type a query, and the AI, contextually aware of their current course, provides tailored explanations. The \textbf{Interactive Quiz System} allows students to take timed quizzes, answer different types of questions, and receive immediate feedback on their performance upon submission. For computer science students, the \textbf{Integrated Code Editor} is a vital tool. It allows them to write, compile, and test code in multiple languages directly within the browser, removing the need for complex local development environment setups.
...
The \textbf{Quiz Schema} is a comprehensive model that supports various question types and detailed submission tracking.

\begin{lstlisting}[language=javascript, caption=Quiz Mongoose Schema]
const mongoose = require('mongoose');
...
The \textbf{Quiz API Endpoints} are segregated based on user roles to ensure proper authorization. The \texttt{teacher.js} router exposes endpoints for full CRUD (Create, Read, Update, Delete) functionality over the quiz models. Teachers can create new quizzes (\texttt{POST /api/v1/teacher/quiz}), retrieve all their quizzes (\texttt{GET /api/v1/teacher/quizzes}), update existing ones (\texttt{PUT /api/v1/teacher/quiz/:id}), and delete them (\texttt{DELETE /api/v1/teacher/quiz/:id}). On the other hand, the \texttt{student.js} router provides endpoints for students to view available quizzes (\texttt{GET /api/v1/student/quizzes}) and submit their answers (\texttt{POST /api/v1/student/submit-quiz/:quizId}). This clear separation of concerns at the API level is crucial for security and maintainability.
...
The quiz functionality is encapsulated in two main React components: `QuizManagement` for teachers and `QuizTaker` for students. The \textbf{QuizManagement} component provides teachers with a comprehensive interface to create, edit, and monitor quizzes. It includes a modal for building quizzes with various question types and a dashboard to view submission results. The \textbf{QuizTaker} component offers students a clean and timed environment to take quizzes. It handles question rendering, answer selection, and submission, and it includes a countdown timer to enforce the time limit. For code questions, it integrates the Monaco Editor to provide a rich coding experience.
...
Learning outcomes also saw a positive trend. The group using the platform, with its combination of the AI Tutor and the new interactive quiz system, showed a 28\% improvement in assessment scores compared to the control group. Qualitative feedback suggests that this is due to the immediate availability of help and the engaging format of the quizzes; students did not have to wait for office hours to clear up confusion, allowing them to maintain momentum in their studies. Teachers also reported a 45\% reduction in administrative overhead, citing the automated attendance and AI-assisted grading features as major time-savers.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{creating_virtual_class.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{live_class_start.png}
    \caption{Starting a Live Session}
    \label{fig:live_class_start}
\end{figure}

\textbf{Creating and Grading Quizzes:} Navigate to the "Quiz Management" tab. Here you can create new quizzes, specifying the title, description, duration, and questions. You can add multiple-choice or code-based questions. Once a quiz is completed by students, you can view the results in the "Submissions" tab. The system automatically grades multiple-choice questions, and you can review and grade the code-based questions.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{quiz_management.png}
    \caption{Quiz Management Dashboard}
    \label{fig:quiz_management}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{ai_tutor.png}
    \caption{AI Tutor Example}
    \label{fig:ai_tutor_example}
\end{figure}

\textbf{Taking a Quiz:} When a teacher posts a quiz, you will receive a notification on your dashboard. Clicking on the notification will take you to the quiz page. The quiz is timed, and you can see the remaining time on the screen. After answering all the questions, click the "Submit" button. You will receive immediate feedback on your performance.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{quiz_taker.png}
    \caption{Taking a Quiz}
    \label{fig:taking_a_quiz}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{virtual_class.png}
    \caption{Video connection fails}
    \label{fig:placeholder}
\end{figure}
